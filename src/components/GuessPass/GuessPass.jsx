...
function GuessPass() {
	...
	useEffect(() => {
		const intervalId = setInterval(() => {
			const newPositions = positions.map((pos, index) => {
				...
				if (
					newPosition.x <= 0 ||
					newPosition.x >= containerSize.width - ballSize
				) {
				...
				if (
					newPosition.y <= 0 ||
					newPosition.y >= containerSize.height - ballSize
				) {
	...
	function clickHandler(e) {
		...
		if (value.toLowerCase() === "isdf@crack") {
			const encrypted = CryptoJS.AES.encrypt("6", "byp4ss").toString();
	...
	return (
		<div className="flex flex-col justify-center items-center gap-4">
			...
			<div
				className="w-96 h-96 border border-black relative"
				ref={containerRef}
			>
