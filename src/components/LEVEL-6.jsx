...
function Level6() {
	...
	const handleTransfer = () => {
		if (pairs.a === pairs.b) {
			...
			blocks[pairs.b][blocks[pairs.b].length - 1] <
			blocks[pairs.a][blocks[pairs.a].length - 1]
		) {
			setMessage(
				`Block ${
					blocks[pairs.b][blocks[pairs.b].length - 1]
			...
			if (compareArrays(blocks[3], [5, 4, 3, 2, 1])) {
				setMessage2(
					`You Successfully Cleared The level in ${count + 1} moves.`
				);
				if (level == 5) {
					// localStorage.setItem('count', 7 );
					// console.log(parseInt(localStorage.getItem('count')));
					const encrypted = CryptoJS.AES.encrypt("7", "byp4ss").toString();
	...
	return value >= 6 ? (
		<>
			<div className="h-12 w-ful bg-green-500 z-10 px-5 flex items-center justify-around ">
				<p className="bg-green-800 text-white px-4 py-1 rounded-lg">
					{" "}
			...
			<div className="w-auto bg-red-400 m-5 text-center font-bold">
				{message}
			</div>
			<div className="w-auto bg-green-500 m-5 text-center font-bold">
				{message2}
			</div>
			...
			<div className="h-12 w-ful bg-green-500 z-10 px-14 flex items-center gap-14 justify-center  ">
				<Button
					title={`Transfer   ${pairs.a} to ${pairs.b}`}
					txtCol="white"
					bgCol="black"
					onClick={() => setPairs({ a: NaN, b: NaN })}
				/>
				<Button
					title="Transfer"
					txtCol="white"
			...
			<div className="flex justify-center items-center flex-col my-4">
				...
				<ul className="text-center space-y-2">
					<li>
						1. Just click on the block from where you want to move the number
						block{" "}
					</li>
					<li>2. And Click any of the remaining block to tranfer the block</li>
					<li>
						3. For Eg : If you click on block 1 and then on 2 then the Num Block
						will move from from 1 to 2 by using transfer button{" "}
					</li>
					<li>
						4. In order to win this game you just have to put all number block
						in ascending order from top in{" "}
						<i className="font-bold">BLOCK NO 3</i>{" "}
					</li>
				</ul>
			</div>
		</>
			...
			<div className="text-center">
				...
				<p className="text-2xl font-bold">
					<span className="bg-clip-text text-transparent bg-gradient-to-r from-green-400 via-pink-900 to-blue-500">
						Keep playing and try to crack the levels to unlock more fun! {value}
					</span>
				</p>
			</div>
		</div>
