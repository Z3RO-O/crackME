...
const Puzzle = () => {
	...
	const isGameFinished =
		tiles.every((tile, index) => tile === index + 1) && tiles[15] === 0;
	...
	const handleTileClick = (index) => {
		...
		if (isGameFinished3()) {
			...
			const encrypted = CryptoJS.AES.encrypt("10", "byp4sss").toString();
	...
	return value >= 9 ? (
		...
		>
			<div className=" text-black text-3xl flex flex-col mt-16 justify-center items-center h-16 bg-orange-700 ">
				15 PuZZlE GaMe
			</div>
			...
			<div className="flex justify-center items-center w-1/2 ml-96">
				...
				<div className="w-96 border rounded-2xl text-white p-4 mt-20 bg-blue-700">
					The 15 puzzle game involves arranging numbered tiles within a 4x4 grid
					by sliding them into the empty space. The objective is to order the
					tiles sequentially, with the empty space in the bottom right corner
				</div>
			</div>
